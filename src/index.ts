 ...
 */
function collectSections(node: any, map: Map<string, any>) {
 ...
 */
function buildSectionMap(structure: any) {
...
function rangesOverlap(
  ...
  if (fs && effectiveItemEnd < fs) return false;
  if (fe && effectiveItemStart > fe) return false;
  return true;
}
 ...
 */
async function fetchStructureJson(date: string, title: number, part?: string) {
  const path = `/api/versioner/v1/structure/${encodeURIComponent(
    date
  )}/title-${encodeURIComponent(String(title))}.json`;
 ...
 */
async function fetchTitleMeta(title: number) {
  const url = new URL("/api/versioner/v1/titles.json", ECFR_BASE);
  const data = await fetchJson(url.toString());
  const meta =
    Array.isArray((data as any)?.titles) &&
    (data as any).titles.find((t: any) => String(t?.number) === String(title));
  return meta ?? null;
}
 ...
 */
async function diffTitleStructures(
  title: number,
  start_date: string,
  end_date: string,
  part?: string,
  ...
  for (const [id, node] of endMap.entries()) {
    ...
    if (previous) {
      const differs =
        node?.label_description !== previous.label_description ||
  ...
  const filteredChanges = change_types
    ? changes.filter((c) => (change_types as string[]).includes(c.type))
    : changes;
...
async function main() {
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        ...
        const data = await fetchJson(url.toString());
        const corrections: any[] = Array.isArray((data as any)?.ecfr_corrections)
          ? (data as any).ecfr_corrections
          : [];

        const filtered = corrections.filter((c) => {
          if (title && String(c?.title) !== String(title)) return false;
          const corrDate = c?.error_corrected as string | undefined;
          const occDate = c?.error_occurred as string | undefined;
          if (error_corrected_date && corrDate !== error_corrected_date) return false;
          if (date && corrDate !== date && occDate !== date) return false;
          return true;
        });
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      const { title, issue_date_gte, issue_date_lte, after_date, before_date, part, section } =
        args;
      try {
        const path = `/api/versioner/v1/versions/title-${encodeURIComponent(
          String(title)
        )}.json`;
        const url = new URL(path, ECFR_BASE);
        ...
        const versions: Array<{
          ...
          issue_date?: string;
          volume?: number;
        }> = Array.isArray((data as any)?.content_versions)
          ? (data as any).content_versions
          : [];
        ...
        return errorText(
          `Error fetching eCFR title versions: ${err?.message || String(err)}`
        );
      }
    }
  );
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        const path = `/api/versioner/v1/structure/${encodeURIComponent(
          date
        )}/title-${encodeURIComponent(String(title))}.${extension}`;
        const url = new URL(path, ECFR_BASE);
        if (part) url.searchParams.set("part", part);
        ...
        return errorText(
          `Error fetching eCFR title structure: ${err?.message || String(err)}`
        );
      }
    }
  );
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        ...
        return errorText(
          `Error fetching eCFR title ancestry: ${err?.message || String(err)}`
        );
      }
    }
  );
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        ...
        return errorText(
          `Error calling eCFR search with date range: ${err?.message || String(err)}`
        );
      }
    }
  );
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        let targetIndex = structure_index as number | undefined;
        let matchedResult: any | undefined;

        if (!targetIndex) {
          if (!section) {
            return errorText(
              "Provide either structure_index or a section number to locate content."
            );
          }
          const queries = [
            `${title} CFR ${section}`,
          ...
          for (const q of queries) {
            ...
            const candidates: any[] = Array.isArray(searchData?.results)
              ? searchData.results
              : [];
            const filtered = candidates.filter((entry: any) => {
              const matchesSection =
                entry?.hierarchy?.section &&
        ...
        try {
          const path = `/api/versioner/v1/content/${encodeURIComponent(
            date
          )}/${encodeURIComponent(String(targetIndex))}`;
          const url = new URL(path, ECFR_BASE);
          const res = await fetch(url.toString());
          if (!res.ok) {
            const body = await res.text().catch(() => "");
          ...
          return jsonText({
            ...
            citation:
              matchedResult?.hierarchy?.section && matchedResult?.hierarchy?.title
          ...
          if (section) {
            try {
              const xmlPath = `/api/versioner/v1/full/${encodeURIComponent(
                date
              )}/title-${encodeURIComponent(String(title))}.xml`;
              const xmlUrl = new URL(xmlPath, ECFR_BASE);
              if (part) xmlUrl.searchParams.set("part", part);
              ...
              if (!res.ok) {
                const body = await res.text().catch(() => "");
                throw new Error(
                  `HTTP ${res.status} ${res.statusText} from ${xmlUrl.toString()}` +
                    (body ? ` — body: ${body.slice(0, 500)}` : "")
                );
              }
              const xml = await res.text();
              ...
              return errorText(
                `Error fetching eCFR section content: ${xmlErr?.message || err?.message || String(err)}`
              );
            }
          }
          return errorText(
            `Error fetching eCFR section content: ${err?.message || String(err)}`
          );
        }
      } catch (err: any) {
        return errorText(
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        ...
        const latestIssue = meta?.latest_issue_date as string | undefined;
        const effectiveEnd =
          latestIssue && end_date > latestIssue ? latestIssue : end_date;
        const { summary, changes } = await diffTitleStructures(
          title,
          start_date,
          effectiveEnd,
          part,
        ...
        return errorText(
          `Error comparing eCFR title snapshots: ${err?.message || String(err)}`
        );
      }
    }
  );
  ...
  anyServer.tool(
    ...
    async (args: any) => {
      ...
      try {
        ...
        const end =
          latestIssue && requestedEnd > latestIssue ? latestIssue : requestedEnd;
        ...
        return errorText(
          `Error fetching recent eCFR changes: ${err?.message || String(err)}`
        );
      }
    }
  );
